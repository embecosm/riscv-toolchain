...
case "${TARGET_BOARD}" in
    ...
    case "${TARGET_BOARD}" in
        ...
        riscv-coreplexip-e31-arty|riscv-coreplexip-e51-arty|riscv-coreplexip-e31-arty-rtos)
            IGNORE_TESTS="--ignore 'store.exp break.exp mi-break.exp mi-nonstop-exit.exp mi-exit-code.exp mi-simplerun.exp condbreak.exp finish.exp func-ptrs.exp funcargs.exp macscp.exp sepdebug.exp dw2-dir-file-name.exp return2.exp ovldbreak.exp temargs.exp inline-cmds.exp bp-permanent.exp commands.exp'"
            ;;
    esac
